...
model-evaluation products for the Huang2013 and S4G mock campaigns.

The refresh updated existing campaign products from already-written image,
profile, and model files. It did **not** refit the arms.

Most implementation code lives in the sibling `isoster` repository, because
that repository owns the exhausted-benchmark runners, QA plotting code,
inventory schema, scoring code, and model-evaluation logic.

## Campaign Roots

The refreshed campaign products live outside this repository:
...
The refresh rewrote or regenerated:
...
## Implementation Files In `isoster`
...
Refresh and analysis logs were written in the sibling `isoster` repository
under `outputs/`:
