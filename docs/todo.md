...
| P8.9 | P8 | Validation: render 3 vs S4G `_subcomps.fits` cubes | Done | `inputs/cs4g/scripts/qa_mockgal_vs_s4g.py` → `output/cs4g_s4g_irac1_test/qa_s4g_validation.png` + `qa_summary.json`. Shape corr 0.88–0.9996 across NGC1433 / NGC0275 / NGC1357. Uncovered two real issues: (a) manifest `defaults:` block ignored by `load_image_configs`; (b) 0.29–0.54 mag photometric offset per galaxy traced to distance-catalog vs Hubble-flow cosmology mismatch in `cs4g_to_mockgal.py`. Logged as P8.10–P8.12. |
| P8.10 | P8 | Fix manifest `defaults:` merging in `mockgal.load_image_configs` | TODO | See `docs/LESSON.md` → Run-Manifest Loader Behavior. Either merge `defaults` into each `configs` entry in `load_image_configs`, or drop the `defaults` pattern and require flat `configs` rows. Impacts all CS4G run manifests; Huang2013 is unaffected (uses dedicated runner). |
| P8.11 | P8 | Fix CS4G `abs_mag` distance convention | TODO | In `inputs/cs4g/scripts/cs4g_to_mockgal.py`, replace Salo's catalog distance modulus with `FlatLambdaCDM(H0=70, Om0=0.3).distmod(z)` (same cosmology mockgal renders with). Re-emit `cs4g_components.json` + `inputs/cs4g/models/cs4g_sample_models.yaml`. Re-run `qa_mockgal_vs_s4g.py` to confirm flux ratios approach 1.0. Expect no shape changes (corr already 0.88–0.9996). See `docs/LESSON.md` → CS4G Photometric Round-Trip. |
| P8.12 | P8 | Composite-PSF follow-up for CS4G | TODO | Optional: switch mockgal's CS4G manifest from `psf_type: gaussian, psf_fwhm: 1.66` to `psf_type: image, psf_file: output/cs4g_s4g_irac1_test/{name}/s4g_psf_composite.fits`. P8.9 showed peak_ratio=5.3× for NGC1357's pure-Sersic bulge vs Gaussian approximation; composite PSF should tighten peaks. Re-run QA and compare. Low priority — only matters when core residuals dominate the comparison. |
