...
| P8.12 | P8 | Composite-PSF follow-up for CS4G | TODO | Optional: switch mockgal's CS4G manifest from `psf_type: gaussian, psf_fwhm: 1.66` to `psf_type: image, psf_file: output/cs4g_s4g_irac1_test/{name}/s4g_psf_composite.fits`. P8.9's NGC1357 peak_ratio=3.98 reflects the Gaussian-FWHM=1.66″ vs Salo's composite PSF core; composite PSF should tighten peaks without changing flux_ratio or corr. Low priority — only matters when core fidelity is the point. |
